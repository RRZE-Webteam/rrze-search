...
<div id="">
    ...
    if (isset($results['queries']['previousPage'])) {
        echo '<a href="'.site_url().$pageLink.'?q='.urlencode($_GET['q']).'&se='.$_GET['se'].'&start='.$results['queries']['previousPage'][0]['startIndex'].'">'.__('Previous Page', 'rrze-search').'</a>';
    }
    if (isset($results['queries']['previousPage'], $results['queries']['nextPage'])) {
        echo '&nbsp;|&nbsp;';
    }
    if (isset($results['queries']['nextPage'])) {
        echo '<a href="'.site_url().$pageLink.'?q='.urlencode($_GET['q']).'&se='.$_GET['se'].'&start='.$results['queries']['nextPage'][0]['startIndex'].'">'.__('Next Page', 'rrze-search').'</a>';
    } ?>
</div>